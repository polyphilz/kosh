...
interface TidbitComposerProps {
  autoFocus?: boolean;
  contextKey?: string;
  onFileDialogOpenChange?: (open: boolean) => Promise<void> | void;
  onCancel: () => void;
...
export const TidbitComposer = forwardRef<TidbitComposerHandle, TidbitComposerProps>(
  function TidbitComposer(
    {
      autoFocus = false,
    ...
    const contextKey = contextKeyOverride ?? (tidbit ? `edit:${tidbit.id}` : "capture");
    ...
    const [fileDropListenerReady, setFileDropListenerReady] = useState(false);
    ...
    useEffect(() => {
      ...
      return () => {
        active = false;
        setFileDropListenerReady(false);
        unlisten?.();
        void backend
          .setFileDropConsumerActive(false)
          .catch((reason: unknown) =>
            console.error("Could not release the file drop consumer", reason),
          );
      };
    }, [backend, enqueueDraftSave]);

    useEffect(() => {
      if (!("__TAURI_INTERNALS__" in window) || !fileDropListenerReady) {
        return;
      }
      void backend
        .setFileDropConsumerActive(ready && !busy)
        .catch((reason: unknown) =>
          console.error("Could not update the file drop consumer", reason),
        );
      return () => {
        void backend
          .setFileDropConsumerActive(false)
    ...
    }, [backend, busy, fileDropListenerReady, ready]);
    ...
    const submit = async (event?: FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        setError(
          `The tidbit was saved, but its detail view could not open: ${errorMessage(reason)}`,
        );
      }
    };
    ...
    return (
      <>
        <form
          className={classNames("capture-card", `capture-card--${variant}`)}
        ...
        >
          ...
          <RichTextEditor
            ...
            pickImage={async () => {
              await onFileDialogOpenChange?.(true);
              try {
                ...
                await onFileDialogOpenChange?.(false);
              }
            }}
            pickAttachment={async () => {
              ...
              try {
                ...
                if (!selectionId) return null;
                const draft = await enqueueDraftSave(stateRef.current);
                return backend.ingestSelectedAttachment(selectionId, draft.id);
              } finally {
                await onFileDialogOpenChange?.(false);
              }
            }}
            ...
            <Button
              ...
              onClick={() =>
                markChanged((current) => ({
                  ...
                  sources: [
                    ...current.sources,
                    { key: nextSourceKey.current++, label: "", url: "" },
                  ],
                }))
              }
              size="compact"
